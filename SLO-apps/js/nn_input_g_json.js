function clearBrain()
{
	...
	document.getElementById("voorspelling").innerHTML="";
	document.getElementById("lijnen").innerHTML="";	
	document.getElementById("resultaatTraining").innerHTML="";
	document.getElementById("iteraties").innerHTML="";
	document.getElementById("voorspeld").innerHTML="";	
}

function startBrain()
{
	clearBrain();
	let sk = document.getElementById("startknop");
	let bgsk='#ffffff';
	try{
	...
	if(sk)
	{
		setTimeout(function() {sk.style.backgroundColor=bgsk; }, 1);
		console.log("ready color")
	}
}
...
{
	document.getElementById("voorspeld").innerHTML="";
	var restrainingobj=document.getElementById("resultaatTraining");
	var lijnenobj=document.getElementById("lijnen");
	var voorspellingobj=document.getElementById("voorspelling");
	var iteratiesobj=document.getElementById("iteraties");
	var statusobj=document.getElementById("status");
	var diagramobj=document.getElementById("xydiagram");
	...
	logger("<h2>Iteraties</h2>",iteratiesobj);

	// train het netwerk
	var trained = net.train(trainingData, 
		{
			log: (error) => logger(JSON.stringify(error)+"<br/>",iteratiesobj),
	...
	logger("<h2>Status netwerk</h2>JSON<pre> "+JSON.stringify(jsonobj, null, 2)+"</pre>",statusobj);
	logger(toonVoorspellingen(config),voorspellingobj);

	// toon de status van de gewichten als lineaire vergelijking
	logger(
		(goed?"":"<span style='background-color: red; color: yellow'> Niet gelukt: </span> ")+
		"De training stopte na  "+ trained.iterations+" iteraties met fout "+ trained.error,restrainingobj)
	logger("<h2>Voorspelde vergelijkingen</h2>",lijnenobj);
	...
	setWidthElement(document.getElementById("tabs"),diagramobj,restrainingobj);
	lijnenobj.style.display="block";
	//setWidthElement(document.getElementById("tabs"),restrainingobj.offsetWidth-10);
}
